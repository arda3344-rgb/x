...
module.exports = {
  ...
  snekfetch
    .get(
      `https://discordbots.org/api/bots/${client.user.id}/check?userId=${message.author.id}`
    )
    ...
    .then(response => {
      ...
      if (check == 1) {
        message.channel
          .send(
            "<a:yukleniyor2:796015218814877778> Kapak Laf Yükleniyor..<a:yukleniyor2:796015218814877778> "
          )
          .then(message => {
            var espriler = [
        ...
        let embed = new Discord.RichEmbed()
          ...
          .setDescription(
            ...
            )} **Hata**, bu komutu kullanmak için **12 saat aralıkla** **[BURADAN](https://discordbots.org/bot/${
              client.user.id
            }/vote)**  botu oylamanız gerekmektedir. Onaylanması **1-4** dakikayı bulabilir, lütfen bekleyin. `
          );
